...
print("Testing with num_predict=2000...")
llm1 = ChatOllama(model="qwen3:4b", base_url="http://localhost:11434", temperature=0.4, num_predict=2000, num_ctx=8192, top_p=0.9)
try:
    print(repr(llm1.invoke([SystemMessage(content=CRITIC_SYSTEM_PROMPT), HumanMessage(content=CRITIC_HUMAN_PROMPT)]).content))
except Exception as e:
    print(e)

print("Testing with no num_predict...")
llm2 = ChatOllama(model="qwen3:4b", base_url="http://localhost:11434", temperature=0.4, num_ctx=8192, top_p=0.9)
try:
    print(repr(llm2.invoke([SystemMessage(content=CRITIC_SYSTEM_PROMPT), HumanMessage(content=CRITIC_HUMAN_PROMPT)]).content))
except Exception as e:
    print(e)
